...
sys.path.append('../bin')
import netCDF4 
from track_functions import get_drifter, get_fvcom, get_roms, draw_basemap, distance, uniquecolors # all homegrown functions needed for this routine
...
option=3
numpts=2 # needed for case where option==3
...
colors=uniquecolors(len(drifter_ids))
inc=-1
for ID in drifter_ids:
    ...
    if (option!=1) and (ID>0):
       if lonsize>finallonsize:
            finallonsize=lonsize
       if latsize>finallatsize:
            finallatsize=latsize
    if (ID==0) | (option==1): # case of multiple start positions specified
       finallonsize=lonsize
       finallatsize=latsize
       fig = plt.figure()
    ...
    if MODEL in ('FVCOM', 'BOTH'):
        ...
        ax.plot(points_fvcom['lon'],points_fvcom['lat'],'o-',color=colors[inc],label='fvcom')
